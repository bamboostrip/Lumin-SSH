...
import {
  AI_COLLABORATION_COMPRESSION_PREFIX, AI_COLLABORATION_CONTINUE_PREFIX, AI_COLLABORATION_DONE_PREFIX, AI_COLLABORATION_RETRY_PREFIX, AI_CONVERSATION_DIFF_SUCCESS_STATUSES, AI_CONVERSATION_DIFF_TOOL_NAMES, AI_FOLLOWUP_COMPLETED_STATUS_KEY, AI_FOLLOWUP_PENDING_STATUS_KEY, buildAIFollowupAnswerPayload, buildAIQueuedSubmission, buildMetrics, buildReasoningDuration, cloneAIConversationCacheObjects, computeAILastAssistantTurnState, createAPIHistoryMessage, findApiAnchorIndexByUiMessageId, findLatestAIFollowupMessageByRequestId, getAIBridge, insertMessageBeforeAssistant, isAIBusinessTurnMessageKind, normalizeAICollaborationDecision, normalizeAICollaborationMode, normalizeAIContextTokensValue, normalizeAIMessageStatus, normalizeAIRuntimePhase, parseAICollaborationStreamBuffer, resolveAIEventSound, trimLatestAssistantAPIHistoryMessage, updateAILastAssistantTurnState, upsertAPIHistoryMessage, upsertMessageBeforeAssistant,
} from './aiChatLogic.ts'
import type { AIConversationSnapshot, AIMessage, AIToolExecution, APIHistoryMessage, PanelState } from './aiChatLogic.ts'
...
// ai-chat-stream 流式事件总 reducer（runtime_phase / collaboration_* / assistant_* /
// tool_* / delta / done / error / cancelled 等分支）。
// 从 AIConversationTabPanel 原样搬移；闭包依赖经 deps 注入，保持同名解构、代码零改动。
export interface AIChatStreamEventsDeps {
  ...
  setPanelState: (panelKey: string, updater: ((current: PanelState) => PanelState) | Partial<PanelState>) => void
  enrichAIChatCommandMessage: (message: AIMessage) => AIMessage
  playAISound: (type: string) => void
  rebuildAIConversationTokenLedger: (snapshot: AIConversationSnapshot, targetPanelKey?: string) => Promise<unknown>
  saveConversationSnapshot: (snapshot: AIConversationSnapshot, targetPanelKey?: string, options?: { hydrate?: boolean }) => Promise<unknown>
  resumeAIChatFromConversation: (conversationSnapshot: AIConversationSnapshot, targetPanelKey?: string, options?: Record<string, unknown>) => Promise<unknown>
  runAIConversationSummarySubtaskFlow: (conversationSnapshot: AIConversationSnapshot, options?: Record<string, unknown>) => Promise<unknown>
