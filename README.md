...
- **可选加密备份** — 设置恢复密码后使用 AES-256-GCM 生成 `.enc` 密文备份；未设置时使用便于迁移的 `.json` 快照
...
设置 → 同步与云 → 配置 WebDAV / R2 / FTP / SFTP 任一后端。需要密文备份时请先设置恢复密码，Lumin 会生成 AES-256-GCM 加密的 `.enc` 快照；未设置恢复密码时同步 `.json` 快照。在新设备上配置同一后端即可恢复。
