...
func (m *SSHManager) GetTerminalCwd(sessionId string) (string, error) {
	...
	cmd := fmt.Sprintf(`CUR_PID=$$; SSHD_PID=""; while [ -n "$CUR_PID" ] && [ "$CUR_PID" -gt 1 ]; do CUR_PID=$(ps -o ppid= -p "$CUR_PID" 2>/dev/null | tr -d ' '); if [ -z "$CUR_PID" ]; then break; fi; if ps -o comm= -p "$CUR_PID" 2>/dev/null | grep -q sshd; then SSHD_PID=$CUR_PID; break; fi; done; SHELL_PID=""; MY_MNT=$(readlink /proc/$$/ns/mnt 2>/dev/null); if [ -n "$SSHD_PID" ]; then for child in $(pgrep -P "$SSHD_PID" 2>/dev/null); do if [ "$child" != "$$" ] && [ "$child" != "$PPID" ]; then if ps -o comm= -p "$child" 2>/dev/null | grep -qE 'sh|bash|zsh'; then PID_MNT=$(readlink /proc/$child/ns/mnt 2>/dev/null); if [ -z "$MY_MNT" ] || [ -z "$PID_MNT" ] || [ "$MY_MNT" = "$PID_MNT" ]; then SHELL_PID=$child; break; fi; fi; fi; done; fi; if [ -z "$SHELL_PID" ]; then PORT=%s; SSHD_PID_PORT=$(ss -ntp 2>/dev/null | grep ":$PORT " | grep -oE 'pid=[0-9]+' | cut -d= -f2 | head -n1); [ -z "$SSHD_PID_PORT" ] && SSHD_PID_PORT=$(netstat -ntp 2>/dev/null | grep ":$PORT " | grep -oE '[0-9]+/sshd' | cut -d/ -f1 | head -n1); if [ -n "$SSHD_PID_PORT" ]; then for child in $(pgrep -P "$SSHD_PID_PORT" 2>/dev/null); do if [ "$child" != "$$" ] && [ "$child" != "$PPID" ]; then if ps -o comm= -p "$child" 2>/dev/null | grep -qE 'sh|bash|zsh'; then PID_MNT=$(readlink /proc/$child/ns/mnt 2>/dev/null); if [ -z "$MY_MNT" ] || [ -z "$PID_MNT" ] || [ "$MY_MNT" = "$PID_MNT" ]; then SHELL_PID=$child; break; fi; fi; fi; done; fi; fi; if [ -z "$SHELL_PID" ]; then SHELL_PID=$(pgrep -u "$USER" -f "sh|bash|zsh" 2>/dev/null | while read -r pid; do if [ "$pid" != "$$" ] && [ "$pid" != "$PPID" ]; then PID_MNT=$(readlink /proc/$pid/ns/mnt 2>/dev/null); if [ -z "$MY_MNT" ] || [ -z "$PID_MNT" ] || [ "$MY_MNT" = "$PID_MNT" ]; then echo "$pid"; fi; fi; done | tail -n1); fi; if [ -n "$SHELL_PID" ]; then readlink /proc/$SHELL_PID/cwd 2>/dev/null || echo "/"; else echo "/"; fi`, portStr)
