...
import { normalizeAIWorkspaceReconnectTerminals, remapAIWorkspaceTabSnapshotGroups, type RestoredSnapshotSession } from '../utils/sessionReconnect.ts';
import { useSessionAuthPrompts } from './useSessionAuthPrompts.ts';
...
// RestoredSnapshotSession / normalizeAIWorkspaceReconnectTerminals / remapAIWorkspaceTabSnapshotGroups
// 已移至 utils/sessionReconnect.ts
...
export default function useSessionConnections(deps: UseSessionConnectionsDeps): UseSessionConnectionsResult {
  ...
  const { resolveHostKeyChoice, resolvePasswordPrompt } = useSessionAuthPrompts({
    addToast,
    t,
